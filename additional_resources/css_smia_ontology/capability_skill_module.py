...
from aas_ontology_reader.aas_ontology_reader_info import AASOntologyReaderInfo
from aas_ontology_reader.utilities import OntologyUtils

css_ontology = get_ontology('<add ontology OWL file path>')
base_namespace = css_ontology.get_namespace('<add your namespace>')
...
class ExtendedThing(Thing):
    ...
    def get_associated_aas_class(cls):
        ...
        """
        if cls.iri not in AASOntologyReaderInfo.CSS_ONTOLOGY_AAS_MODEL_LINK:
            ...
            return AASOntologyReaderInfo.CSS_ONTOLOGY_AAS_MODEL_LINK[cls.iri]
    ...
    def seek_associated_data_properties(self):
        ...
        for prop in css_ontology.properties():
            if isinstance(prop, DataPropertyClass):
                if OntologyUtils.check_whether_part_of_domain(self, prop.domain):
                    ...
                    for range_value in prop.range:
                        if isinstance(range_value, DatatypeClass):
                            possible_values = OntologyUtils.get_possible_values_of_datatype(range_value)
                            xsd_value_type = OntologyUtils.check_and_get_xsd_datatypes(range_value)
    ...
    def check_valid_data_property_value(self, data_property_name, data_property_value):
        ...
        if data_property_name not in self.data_properties_types_dict:
            print("The data property {} does not exist in this OWL class ({}).".format(data_property_name,
                                                                                                 self))
        else:
            ...
            if isinstance(data_property_type, set):
                if data_property_value not in data_property_type:
                    print("The data property value {} for the OWL class {} is not within the valid values."
                          "".format(data_property_value, self))
            else:
                if not isinstance(data_property_value, data_property_type):
                    print("The data property value {} for the OWL class {} is not valid.".format(
                        data_property_value, self))
    ...
    def get_data_property_name_by_iri(self, property_iri):
        ...
        for prop in self.data_properties_dict:
            if prop.iri == property_iri:
                return prop.name
        print("The data property with IRI {} does not exist in class {}".format(property_iri, self))
        return None
...
class Capability(ExtendedThing):
    ...
    def check_instance(self):
        ...
        if self.has_lifecycle is None:
            print("The 'has_lifecycle' attribute is required in Capability instances.", self)
        for skill in self.isRealizedBy:
            if not isinstance(skill, Skill):
                print("The instance {} is added in 'isRealizedBy' and it is not a Skill".format(skill))
